...
from CEPC.src.ETweet import ELoadType
...
class EBertConfig:
    ...
    def get_config(cmd, cls_type, model_path, model_path_2, lm_model_path,
        ...
        config = EBertConfig(cmd=cmd, cls_type=cls_type, bert_config=b_config, label_count=b_config.num_labels,
                             ...
                             max_seq=EVar.MaxSequence, batch_size=EVar.BertBatchSize, epoch_count=epoch_count,
                             seed=seed, learn_rate=learn_rate, early_stopping_patience=epoch_count * 10,
                             max_grad_norm=1.0, weight_decay=0.0, adam_epsilon=1e-8, warmup_steps=0,
                             train_by_log_softmax=False, training_log_softmax_weight=1, training_softmax_temperature=1,
...
class EInputBundle:
    ...
    def append(tgt_bundle, src_bundle, tws):
        ...
        for tw_ind, cur_ind in enumerate(to_append):
            ...
            tgt_bundle.tws[-1].Tweetid = tgt_bundle.tws[-1].Tweetid + '-' + str(tw_ind)
        ELib.PASS()
    ...
    def get_data(label_count, lc, train_path_nullable, valid_path_nullable, test_path_nullable,
                 ...
                 tokenize_by_etokens=False, pivot_query=None, load_etokens=True, max_set_length = 0):
        train_bundle, valid_bundle, test_bundle, unlabeled_bundle, = [None] * 4
        load_type = ELoadType.stored_tags
        if not load_etokens:
            load_type = ELoadType.none
        if train_path_nullable is not None:
            tws_train = ETweet.load(train_path_nullable, load_type)
        ...
        if unlabeled_path_nullable is not None:
            tws_unlabeled = ETweet.load(unlabeled_path_nullable, load_type)
    ...
    @staticmethod
    def populate_bundle(bundle, size, seed, lc=None):
        ...
        if lc is not None:
            ...
            tws = ETweet.random_sample(bundle.tws, ratio_needed, seed, True)
...
class EDomainAdaptParam:

    def __init__(self):
        ## custom parameters
        self.cache_dir = None
        self.cache_global = None
        self.cache_iteration = None
        self.flag = None
        self.labeled_domains = None
        self.unlabeled_domains = None
        self.tag = None
        ## param ranges
        self.Mine = [0.97, 0.96, 0.95, 0.94]
        self.__param = self.Mine
        self.__param_ind = 0
        ELib.PASS()
    ...
    def val(self):
        return self.__param[self.__param_ind]

    def report(self):
        return '[{}/{}] The hyper-parameters were: {}'.format(self.__param_ind + 1, len(self.__param), self.val())
