...
class ETweet:
    ...
    @staticmethod
    def random_sample(tws, ratio, seed, with_replacement=False):
        random.seed(seed)
        if with_replacement:
            result = random.choices(tws, k=ceil(len(tws) * ratio))
        else:
            result = random.sample(tws, k=ceil(len(tws) * ratio))
        random.shuffle(result)
        return result
    ...
    @staticmethod
    def remove_labels(tws, lc):
        for cur_tws in tws:
            cur_tws.Label = lc.negative_new_label.source_lbls[0]
            ELib.PASS()
        ELib.PASS()
